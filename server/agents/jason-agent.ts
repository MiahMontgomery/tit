...
export class JasonAgent {
  ...
  public async handleUserRequest(request: string, projectId?: string): Promise<string> {
    ...
    // Check if this is a persona creation request
    if (analysis.type === 'persona_creation') {
      return await this.handlePersonaCreation(request, projectId);
    }
  ...
  private async handlePersonaCreation(request: string, projectId?: string): Promise<string> {
    try {
      console.log('🎭 Jason: Creating persona project...');
      
      const { personaCreator } = await import('../services/persona-creator');
      
      // Parse the request to determine persona type and details
      const personaType = this.determinePersonaType(request);
      const personaName = this.extractPersonaName(request);
      const description = this.extractDescription(request);
      
      // Create the persona project
      const personaProject = await personaCreator.createPersonaProject(
        ...
        {
          name: personaName,
          traits: this.extractTraits(request),
          communicationStyle: this.extractCommunicationStyle(request),
          expertise: this.extractExpertise(request),
          targetAudience: this.extractTargetAudience(request)
        }
      );
      ...
      this.addMemory({
        type: 'decision',
        content: `Created ${personaType} persona project: ${personaName}`,
        context: { 
          personaProject,
          request,
      ...
      return `I've successfully created a ${personaType} persona project called "${personaName}". The project has been set up with API keys and is ready to begin autonomous operation. You can find it in the project dashboard.`;
      
    } catch (error) {
      console.error('Error creating persona project:', error);
      return `I encountered an error while creating the persona project: ${error.message}. Please try again with more specific details.`;
    }
  }
